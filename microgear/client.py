...
def connect(block=False):
    ...
    password = hmac(
        microgear.accesstoken["secret"] + "&" + microgear.gearsecret,
        microgear.accesstoken["token"] + "%" + username,
    )
...
def auto_subscribeAndpublish():
    ...
    if microgear.mqtt_client:
        for topic in publish_list:
            microgear.mqtt_client.publish(
                topic[0], topic[1], retain=topic[2].get('retain', False)
            )
            publish_list = []
    else:
        on_error("Microgear currently is not available.")
...
def subscribe(topic, qos=0):
    ...
    if microgear.mqtt_client:
        t = threading.Thread(
            target=subscribe_thread, args=("/" + microgear.appid + topic, qos)
        )
        threads.append(t)
        t.start()
    else:
...
def publish(topic, message, args={}):
    ...
    if microgear.mqtt_client:
        t = threading.Thread(
            target=publish_thread, args=("/" + microgear.appid + topic, message, args)
        )
        threads.append(t)
        t.start()
    else:
...
def get_token():
    ...
    if cached:
        ...
        cached = cache.set_item(
            "microgear-" + microgear.gearkey + ".cache", {"key": microgear.gearkey}
        )
...
def get_requesttoken(cached):
    ...
    if microgear.securemode:
        url = (
            "https://"
            + microgear.gearauthsite
            + ":"
            + microgear.gearapisecureport
            + path
        )
    else:
...
def get_accesstoken(cached):
    ...
    path = (
        ...
        + microgear.requesttoken.get("token")
        + "&client_id="
        + microgear.gearkey
        + "&client_secret="
        + microgear.gearsecret
        + "&state=mgrev:"
        + microgear.mgrev
    )
    if microgear.securemode:
    ...
    if response.status_code == 200:
        ...
        revokecode = hmac(token[1] + "&" + microgear.gearsecret, token[0]).replace(
            '/', '_'
        )
...
def resettoken():
    ...
    if cached:
        ...
        if "revokecode" in microgear.accesstoken:
            path = (
                "/api/revoke/"
                + microgear.accesstoken["token"]
                + "/"
                + microgear.accesstoken["revokecode"]
            )
            url = ""
            if microgear.securemode:
                url = (
...
def writeFeed(feedid, data, feedkey=""):
    if len(feedid) > 0 and isinstance(data, dict):
        json = "{"
        for key in data:
